...
class VideoWriter:
    ...
    def __init__(self, filename, fps, shape):
        ...
        self.pipe = sp.Popen(
            cmd, stdin=sp.PIPE, stdout=sp.DEVNULL, stderr=sp.DEVNULL, bufsize=10**9
        )
...
def kill_proc(proc):
    proc.kill()
    logger.info("Process running overtime! Killed.")
...
def combine_video_audio(src_video, src_audio, dst_video, verbose=False):
    try:
        ...
        if verbose:
            logger.info("Processed:{}".format(dst_video))
    except Exception as e:
        logger.info("Error:[{}] {}".format(dst_video, e))
