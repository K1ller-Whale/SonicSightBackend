...
class ModelBuilder:
    ...
    def build_sound(self, arch="unet5", fc_dim=64, weights=""):
        ...
        if len(weights) > 0:
            logger.info("Loading weights for net_sound")
            net_sound.load_state_dict(torch.load(weights, map_location="cpu"))
    ...
    def build_frame(self, arch="resnet18", fc_dim=64, pool_type="avgpool", weights=""):
        ...
        if arch == "resnet18fc":
            ...
            net = ResnetFC(original_resnet, fc_dim=fc_dim, pool_type=pool_type)
        ...
        if len(weights) > 0:
            logger.info("Loading weights for net_frame")
    ...
    def build_synthesizer(self, arch, fc_dim=64, weights=""):
        ...
        if len(weights) > 0:
            logger.info("Loading weights for net_synthesizer")
            net.load_state_dict(torch.load(weights, map_location="cpu"))
        return net
