...
class Unet(nn.Module):
    def __init__(self, fc_dim=64, num_downs=5, ngf=64, use_dropout=False):
        ...
        unet_block = UnetBlock(
            fc_dim, ngf, input_nc=1, submodule=unet_block, outermost=True
        )
...
class UnetBlock(nn.Module):
    def __init__(
        ...
        if outermost:
            ...
            upconv = nn.Conv2d(inner_output_nc, outer_nc, kernel_size=3, padding=1)
            ...
            upconv = nn.Conv2d(
                inner_output_nc, outer_nc, kernel_size=3, padding=1, bias=use_bias
            )
